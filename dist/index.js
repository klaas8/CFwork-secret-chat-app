...
var ChatRoom = class {
  constructor(state, env) {
    this.state = state;
    this.sessions = [];
    this.lastTimestamps = /* @__PURE__ */ new Map();
  }
  async fetch(request) {
    ...
    ws.addEventListener("message", async (msg) => {
      try {
        ...
        if (data.type === "identity") {
          ...
          session.avatar = data.avatar || "\u{1F916}";
        ...
        if (data.type === "typing") {
          this.broadcast({ type: "typing", name: session.name }, session.id);
          return;
        }
        if (data.type === "chat") {
          const now = Date.now();
  ...
  async updateAndBroadcastStatus() {
    ...
    const remaining = 1e5 - dailyWrites;
  ...
  generateName() {
    ...
    const randomNum = Math.floor(Math.random() * 9e3) + 1e3;
