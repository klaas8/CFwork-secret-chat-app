...
export default {
  async fetch(request, env) {
    ...
    if (request.headers.get("Upgrade") === "websocket") {
      ...
      // 对于所有其他 HTTP 请求，我们直接返回 HTML 页面的内容
...
<head>
    ...
    <style>
        ...
        .app-container.logs-open { transform: translateX(300px); }
        .app-container.settings-open { transform: translateX(-300px); }

        .side-panel { position: fixed; top: 0; width: 300px; height: 100%; background-color: #2a2f3a; z-index: 20; display: flex; flex-direction: column; box-shadow: 0 0 15px rgba(0,0,0,0.2); transition: transform 0.3s ease-in-out; }
        .logs-panel { left: 0; transform: translateX(-100%); color: #eee; }
        .settings-panel { right: 0; transform: translateX(100%); background-color: #f8f9fa; padding: 20px; box-sizing: border-box; }
        .app-container.logs-open .logs-panel { transform: translateX(0); }
        .app-container.settings-open .settings-panel { transform: translateX(0); }
        
        .side-panel h3 { margin: 0; padding: 15px; background-color: #353b48; text-align: center; font-size: 16px; flex-shrink: 0; }
        .settings-panel h3 { background-color: #e9ecef; color: #333; }
        #logs-output { flex-grow: 1; overflow-y: auto; padding: 10px; font-family: 'Courier New', Courier, monospace; font-size: 12px; line-height: 1.5; }
        .log-entry { padding: 2px 0; border-bottom: 1px solid #444; word-break: break-all; }
        .log-entry.error { color: #ff6b6b; }
        .log-entry.info { color: #68d391; }
        .log-entry.warn { color: #f9d423; }
...
<body>
    <div class="app-container" id="app-container">
        ...
        <div class="main-content">
            <div class="chat-header">
                <button id="toggle-logs" class="toggle-button">📜</button>
    ...
    <script>
        ...
        function log(message, type = 'log') {
            const originalConsole = console[type] || console.log;
            originalConsole(message);
            const entry = document.createElement('div');
            entry.classList.add('log-entry', type);
            entry.textContent = \`[\${new Date().toLocaleTimeString()}] \${typeof message === 'object' ? JSON.stringify(message) : message}\`;
            ui.logsOutput.appendChild(entry);
            ui.logsOutput.scrollTop = ui.logsOutput.scrollHeight;
        }
        console.log = (msg) => log(msg, 'log');
        console.info = (msg) => log(msg, 'info');
        ...
        function initialize() {
            ...
            ui.toggleLogsButton.addEventListener('click', () => ui.appContainer.classList.toggle('logs-open'));
            ...
            document.addEventListener('click', (e) => {
                if (!ui.emojiPicker.contains(e.target) && e.target !== ui.emojiToggleButton) {
        ...
        function loadSettings() {
            ...
            log(\`加载本地设置: \${JSON.stringify(myIdentity)}\`);
        }
        ...
        function populateAvatars() {
            ...
            avatars.forEach(avatar => {
                ...
                option.addEventListener('click', (e) => {
                    ui.avatarSelector.querySelector('.avatar-option.selected')?.classList.remove('selected');
        ...
        function saveAndIdentify() {
            ...
            log(\`保存新设置: \${JSON.stringify(myIdentity)}\`);
            
            if (socket && socket.readyState === WebSocket.OPEN) {
                sendIdentity();
            } else {
                log('连接未建立，将在连接后自动发送身份信息。', 'warn');
            }
        ...
        function sendIdentity() {
            if (socket && socket.readyState === WebSocket.OPEN) {
                ...
                log(\`发送身份信息: \${JSON.stringify(identityPayload)}\`);
            }
        }
        ...
        function onSocketMessage(event) {
            ...
            switch (data.type) {
                ...
                case 'identity':
                    ...
                    log(\`身份已确认: \${JSON.stringify(myIdentity)}\`, 'info');
                    break;
                case 'typing':
        ...
        function sendMessage() {
            ...
            if (socket && socket.readyState === WebSocket.OPEN && text !== '') {
                ...
                log(\`发送聊天消息: \${JSON.stringify(payload)}\`);
                ui.input.value = '';
            } else {
